...
class MMBattleRecord {
    ...
    var roundsJSON = [JSON]()
    var guardiansJSON = [JSON]()
    
    var resultJSON: JSON?
    
    
    var currentGuardiansJSON = [JSON]()
    var currentRoundStateJSON = [JSON]()
    
    
    var fightState: MMFightState!
    ...
    func putGuardian(_ g: MMGuardianState) {
        self.currentGuardiansJSON.append(g.json)
//        self.currentGuardiansJSON = []
    }
    
    
    func putAffectState(_ state: MMAffectState) {
        self.fightState.affectArrayJSON.append(state.json)
    }
    
    
    func putFightState(_ state: MMFightState) {
...
struct MMFightState {
    ...
    init(fromCharacter char: MMCharacter) {
